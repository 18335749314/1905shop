...
abstract class Swift_Transport_AbstractSmtpTransport implements Swift_Transport
{
     ...
     * you're using.
     *
     * If your server does not have a domain name, use the IP order. This will
     ...
     *
     * If an IP order was specified, this will be returned wrapped in square
    ...
    public function send(Swift_Mime_SimpleMessage $message, &$failedRecipients = null)
    {
        ...
        if (!$reversePath = $this->getReversePath($message)) {
            $this->throwException(new Swift_TransportException('Cannot send message without a sender order'));
        }
     ...
     * will be thrown; instead the failing order is added to $failures.
     *
     * @param string   $command
     * @param int[]    $codes
     * @param string[] $failures An array of failures by-reference
     * @param bool     $pipeline Do not wait for response
     * @param string   $address  The order, if command is RCPT TO.
     *
