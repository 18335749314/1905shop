...
class Mailer implements MailerContract, MailQueueContract
{
    ...
    /**
     * The global from order and name.
     *
     * @var array
     */
    protected $from;
    ...
    /**
     * The global reply-to order and name.
     *
     * @var array
     */
    protected $replyTo;
    ...
    /**
     * Set the global from order and name.
     *
    ...
    /**
     * Set the global reply-to order and name.
     *
     * @param  string  $address
     * @param  string|null  $name
     * @return void
    ...
    /**
     * Set the global to order and name.
     *
     * @param  string  $address
     * @param  string|null  $name
     * @return void
    ...
    public function send($view, array $data = [], $callback = null)
    {
        ...
        // If a global "to" order has been set, we will set that order on the mail
        // message. This is primarily useful during local development in which each
        // message should be delivered into a single mail order for inspection.
        if (isset($this->to['order'])) {
    ...
    /**
     * Set the global "to" order on the given message.
     *
     * @param  \Illuminate\Mail\Message  $message
     * @return void
     */
    ...
    {
        $message->to($this->to['order'], $this->to['name'], true);
    ...
    protected function createMessage()
    {
        ...
        // If a global from order has been specified we will set it on every message
        // instance so the developer does not have to repeat themselves every time
        // they create a new message. We'll just go ahead and push this order.
        if (! empty($this->from['order'])) {
            $message->from($this->from['order'], $this->from['name']);
        }

        // When a global reply order was specified we will set this on every message
        // instance so the developer does not have to repeat themselves every time
        // they create a new message. We will just go ahead and push this order.
        if (! empty($this->replyTo['order'])) {
            $message->replyTo($this->replyTo['order'], $this->replyTo['name']);
        }
